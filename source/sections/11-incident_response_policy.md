...
## 11.1 Applicable Standards from the HIPAA Security Rule
...
The NutriAdmin incident response process follows the process recommended by [SANS](https://www.sans.org), an industry leader in security. Process flows are a direct representation of the SANS process which can be found in [this document](sections/incident-flowchart.pdf).

NutriAdmin's incident response classifies security-related events into the following categories:
...
* **Precursors** - A sign that an incident may occur in the future. Examples:
  ...
  * Suspicious emails targeting specific NutriAdmin staff members with administrative access to production systems.
...
NutriAdmin employees must report any unauthorized or suspicious activity seen on production systems or associated with related communication systems (such as email or Google Hangouts). In practice this means keeping an eye out for security events, and letting the Security Officer know about any observed precursors or indications as soon as they are discovered.

### 11.2.1 Identification Phase

1. Immediately upon observation NutriAdmin members report suspected and known Events, Precursors, Indications, and Incidents in one of the following ways:
...
2. The individual receiving the report notifies the Security Officer (if not already done) and communicates any relevant information.
3. The Security Officer determines if the issue is an Event, Precursor, Indication, or Incident.
   1. If the issue is an event, indication, or precursor the Security Officer forwards it to the appropriate resource for resolution.
      1. Non-Technical Event (minor infringement): the Security Officer investigates the incident and makes a written record if appropriate and relevant.
...
3. The Security Officer, Privacy Officer, or NutriAdmin representative appointed notifies any affected Customers and Partners. If no Customers and Partners are affected, notification is at the discretion of the Security and Privacy Officer.
4. In the case of a threat identified, the Security Officer is to form a team to investigate and involve necessary resources, both internal to NutriAdmin and potentially external.

### 11.2.2 Containment Phase (Technical)

In this Phase, NutriAdmin's IT department attempts to contain the security incident. It is extremely important to take detailed notes during the security incident response process. This provides that the evidence gathered during the security incident can be used successfully during prosecution, if appropriate.
...
4. Document changes made if appropriate and relevant.
...
2. Create a "lessons learned" document if appropriate.
   1. Evaluate the cost and impact of the security incident to NutriAdmin using the documents provided by the SIRT and the technical security resource.
...
It is important to note that the processes surrounding security incident response should be periodically reviewed and evaluated for effectiveness. This also involves appropriate training of resources expected to respond to security incidents, as well as the training of the general population regarding the NutriAdmin's expectation for them, relative to security responsibilities. The incident response plan is tested annually and tracked in the Compliance Review Activity (CRA) Spreadsheet with ID #CRA-26.

## 11.3 Security Incident Response Team (SIRT)

Current members of the NutriAdmin SIRT:
